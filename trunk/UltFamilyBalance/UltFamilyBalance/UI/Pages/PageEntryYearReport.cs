...
{
    public partial class PageEntryYearReport : UserControl, IPage
    {
        ...
        private const string REPORT_DATATABLE_NAME                  = "EntriesByYear";
        private const string REPORT_BY_TYPE_DATATABLE_NAME          = "EntriesByYearAndType";
        ...
        public string Title
        {
            get { return _direction == EntryDirection.OutgoingReference ? "Report Annuale Uscite" : "Report Annuale Entrate"; }
        }
        ...
        private SqlCommand CreateCommandEntryReport(SqlConnection connection, int direction)
        {
            // Query
            string sql = "SELECT   [Year], " + 
                         "         [Count], " +
                         "         [Total] " + 
                         "FROM vwEntriesByYear " +
        ...
        private SqlCommand CreateCommandEntryReportByType(SqlConnection connection, int direction)
        {
            // Query
            string sql = "SELECT   [Year], " + 
                         "         [Count], " +
        ...
        private void BindData()
        {
            if (!_data.Relations.Contains(REPORT_RELATION_NAME))
            {
                ...
                _data.Relations.Add( new DataRelation(REPORT_RELATION_NAME, 
                                                      _data.Tables[REPORT_DATATABLE_NAME].Columns[REPORT_RELATION_YEAR_FIELD], 
                                                      _data.Tables[REPORT_BY_TYPE_DATATABLE_NAME].Columns[REPORT_RELATION_YEAR_FIELD]));
        ...
        public void Init(params object[] args)
        {
            ...
            // Commands creation
            _commandEntryReport = CreateCommandEntryReport(UltFamilyBalance.GetUltFamilyBalance().GetSqlConnection(),_direction.Id);
            _commandEntryReportByType = CreateCommandEntryReportByType(UltFamilyBalance.GetUltFamilyBalance().GetSqlConnection(),_direction.Id);
            // Adapters
